import { h as hash } from "./ohash.mjs";
  ...
  async function get(key, resolver, shouldInvalidateCache, event) {
    ...
    const _resolve = async () => {
      ...
      try {
        ...
        if (!isPending) {
          delete pending[key];
          const evictPromise = _evictFromStorage(key, bases, group, name).catch((error2) => {
            _onError("[cache] Cache eviction error.", error2);
          });
